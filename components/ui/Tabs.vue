    ...
    <section class="pt-32 sm:pt-20">
        <LayoutContainer class="sm:!px-0">
            <div class="flex justify-between items-start gap-16">
                <div class="max-w-[550px] w-full flex flex-col items-start sm:hidden pt-6">
                    <UiSubtitle green v-if="tabs[active].subtitle" :text="tabs[active].subtitle" />
                    <h4 v-if="tabs[active].title" v-html="tabs[active].title"></h4>
                ...
                <nav class="max-w-[460px] sm:max-w-full w-full border-t border-light-gray">
                    ...
                    >
                        <button
                            type="button"
                            class="flex items-center gap-8 sm:gap-6 text-gray font-semibold text-2xl sm:text-xl border-b border-light-gray w-full h-[112px] px-12 sm:px-sm sm:h-20 text-left"
                        ...
                        <div v-if="active === index" class="hidden sm:flex flex-col items-start pt-8 pb-4 px-sm">
                            <UiSubtitle green v-if="tabs[active].subtitle" :text="tabs[active].subtitle" />
                            <h4 v-if="i.title" v-html="i.title"></h4>
